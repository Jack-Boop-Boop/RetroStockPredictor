...
from ..schemas.auth import (
    RegisterRequest, LoginRequest, TokenResponse, UserResponse, UpgradeRequest,
)
...
def _create_default_agent_hierarchy(db: Session, user_id: str) -> None:
    ...
    ceo = CustomAgent(
        id=new_uuid(), user_id=user_id, name="Portfolio CEO",
        agent_type="ceo", parent_id=None, sort_order=0,
    ...
    risk = CustomAgent(
        id=new_uuid(), user_id=user_id, name="Risk Manager",
        agent_type="risk", parent_id=ceo.id, sort_order=1,
        prompt="Evaluate downside risk. Flag extreme volatility or overexposure.",
        weight=1.0,
    )
    quant = CustomAgent(
        id=new_uuid(), user_id=user_id, name="Quant Strategist",
        agent_type="quant", parent_id=ceo.id, sort_order=2,
    ...
    sub_agents = [
        CustomAgent(
            id=new_uuid(), user_id=user_id, name="Technical Analyst",
            agent_type="technical", parent_id=quant.id, sort_order=0,
            prompt="Analyze RSI, MACD, Bollinger Bands, and moving averages for trend signals.",
            weight=1.0,
        ),
        CustomAgent(
            id=new_uuid(), user_id=user_id, name="Fundamental Analyst",
            agent_type="fundamental", parent_id=quant.id, sort_order=1,
            prompt="Evaluate P/E ratio, earnings growth, and balance sheet health.",
            weight=1.0,
        ),
        CustomAgent(
            id=new_uuid(), user_id=user_id, name="Sentiment Analyst",
            agent_type="sentiment", parent_id=quant.id, sort_order=2,
            prompt="Gauge market mood from news flow, social media, and price momentum.",
            weight=1.0,
        ),
        CustomAgent(
            id=new_uuid(), user_id=user_id, name="ML Predictor",
            agent_type="ml", parent_id=quant.id, sort_order=3,
...
def _create_user_defaults(db: Session, user_id: str) -> None:
    ...
    db.add(Portfolio(
        id=new_uuid(), user_id=user_id, name="Paper Trading",
        initial_cash=100000, cash=100000,
    ))
    db.add(Watchlist(
        id=new_uuid(), user_id=user_id, name="Default",
...
def login(body: LoginRequest, db: Session = Depends(get_db)):
    ...
    token = create_access_token(user.id, user.email)
    return TokenResponse(access_token=token, is_guest=user.is_guest)
